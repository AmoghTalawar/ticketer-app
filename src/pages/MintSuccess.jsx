...
const MintSuccess = () => {
  ...
  // Explorer link — Hardhat local has no explorer, Amoy does
  const explorerBase = CHAIN_ID === 31337
    ? null
      ...
      <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '10rem 1rem 6rem 1rem' }}>
        <div style={{ background: '#fff', padding: '3rem', borderRadius: '16px', boxShadow: 'var(--shadow-sm)', width: '100%', maxWidth: '560px', textAlign: 'center' }}>
          ...
          <div style={{ background: '#f8f9fa', padding: '1.5rem', borderRadius: '12px', marginBottom: '2rem', textAlign: 'left' }}>
            ...
            <Row label="Network" value={CHAIN_ID === 31337 ? 'Hardhat Local' : 'Polygon Amoy Testnet'} />
            {txHash && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem' }}>
                ...
                {explorerBase ? (
                  ...
                  <span style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: '#555', wordBreak: 'break-all' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            <button className="btn btn-outline" onClick={() => navigate('/concerts')}>
              Browse More
            </button>
            <button
